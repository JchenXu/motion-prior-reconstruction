...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            preds = self.generator(inp, motion_prior=self.motion_prior, vposer=self.vposer)
            ...
            summary_string = f'[{self.exp_name}]: ({i + 1}/{self.num_iters_per_epoch}) | Total: {bar.elapsed_td} | ' \
    ...
    def validate(self):
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            with torch.no_grad():
                ...
                preds = self.generator(inp, motion_prior=self.motion_prior, vposer=self.vposer, J_regressor=J_regressor)
            ...
            summary_string = f'[{self.exp_name}]: {i + 1}/{len(self.valid_loader)}) | batch: {batch_time * 10.0:.4}ms | ' \
    ...
    def evaluate(self):
        ...
        for i in range(pred_j3ds.shape[0]):
