...
def prepare_output_dir(cfg, cfg_file):
    ...
    # ==== create logdir
    logtime = time.strftime('%m-%d-%H-%M-%S')
    ...
    shutil.copy(src='lib/models/vibe.py', dst=osp.join(logdir, 'vibe.py'))
    shutil.copy(src='lib/models/spin.py', dst=osp.join(logdir, 'spin.py'))

    cfg.LOGDIR = logdir

    # save config
    save_dict_to_yaml(cfg, osp.join(cfg.LOGDIR, 'config_all.yaml'))
