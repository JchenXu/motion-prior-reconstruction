...
def batch_compute_similarity_transform_torch(S1, S2):
    ...
    # 4. Solution that Maximizes trace(R'K) is R=U*V', where U, V are
    # singular vectors of K.
    Us = torch.zeros((0, K.shape[1], K.shape[2]), device=K.device, dtype=K.dtype)
    ss = torch.zeros((0, K.shape[2]), device=K.device, dtype=K.dtype)
    Vs = torch.zeros((0, K.shape[2], K.shape[2]), device=K.device, dtype=K.dtype)
    for i in range(K.shape[0]):
        U, s, V = torch.svd(K[0])
        Us = torch.cat([Us, U[None, ...]], dim=0)
        ss = torch.cat([ss, s[None, ...]], dim=0)
        Vs = torch.cat([Vs, V[None, ...]], dim=0)
