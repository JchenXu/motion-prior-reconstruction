...
class Dataset3D(Dataset):
    ...
    def get_single_item(self, index):
        ...
        if self.dataset_name == '3dpw':
            ...
            kp_3d = self.db['joints3D'][start_index:end_index + 1]
            kp_2d = kp_2d[:, None, ...]
            kp_2d = np.concatenate([kp_2d, kp_2d, kp_2d, kp_2d], axis=1).reshape(seqlen * 4, -1, 3)
            kp_3d = kp_3d[:, None, ...]
            kp_3d = np.concatenate([kp_3d, kp_3d, kp_3d, kp_3d], axis=1).reshape(seqlen * 4, -1, 3)
            seqlen = seqlen * 4
            

        kp_2d_tensor = np.ones((seqlen, 49, 3), dtype=np.float16)
        nj = 14 if not is_train else 49
        kp_3d_tensor = np.zeros((seqlen, nj, 3), dtype=np.float16)
        ...
        if self.dataset_name == '3dpw':
            ...
            if not is_train:
                ...
                w_smpl = torch.zeros(seqlen).float()
                w_3d = torch.ones(seqlen).float()
            else:
                pose = self.db['pose'][start_index:end_index + 1]
                shape = self.db['shape'][start_index:end_index + 1]
                pose = pose[:, None, ...]
                pose = np.concatenate([pose, pose, pose, pose], axis=1).reshape(seqlen, -1) 
                shape = shape[:, None, ...]
                shape = np.concatenate([shape, shape, shape, shape], axis=1).reshape(seqlen, -1)
                w_smpl = torch.ones(seqlen).float()
                w_3d = torch.ones(seqlen).float()
        ...
        if 'h36m' in self.dataset_name:
            bbox = self.db['bbox'][start_index:end_index + 1][:, None, ...]
            bbox = np.concatenate([bbox, bbox, bbox, bbox], axis=1).reshape(seqlen, -1)
            input = torch.from_numpy(self.db['features'][start_index:end_index+1]).float().unsqueeze(1)
            input = torch.cat([input, input, input, input], dim=1).reshape(seqlen, -1)
        else:
            bbox = self.db['bbox'][start_index:end_index + 1]
            input = torch.from_numpy(self.db['features'][start_index:end_index+1]).float()

        theta_tensor = np.zeros((seqlen, 85), dtype=np.float16)
