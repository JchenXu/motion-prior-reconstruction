...
def get_data_loaders(cfg):
    ...
    def get_3d_datasets(dataset_names):
        datasets = []
        for dataset_name in dataset_names:
            if 'H36M' in cfg.TRAIN.DATASET_EVAL:
                db = eval(dataset_name+'_evalh36m')(set='train', seqlen=cfg.DATASET.SEQLEN, debug=cfg.DEBUG)
            elif 'ThreeDPW' in cfg.TRAIN.DATASET_EVAL:
                db = eval(dataset_name+'_eval3dpw')(set='train', seqlen=cfg.DATASET.SEQLEN, debug=cfg.DEBUG)
            else:
    ...
    # ===== Evaluation dataset =====
    if 'H36M' in cfg.TRAIN.DATASET_EVAL:
        valid_db = eval(cfg.TRAIN.DATASET_EVAL+'_evalh36m')(set='test', seqlen=cfg.DATASET.SEQLEN, debug=cfg.DEBUG)
    elif 'ThreeDPW' in cfg.TRAIN.DATASET_EVAL:
        valid_db = eval(cfg.TRAIN.DATASET_EVAL+'_eval3dpw')(set='test', seqlen=cfg.DATASET.SEQLEN, debug=cfg.DEBUG)
    else:
        print('error in dataloader ... ')
        exit()
