...
class Dataset3D(Dataset):
    ...
    def load_db(self):
        ...
        if osp.isfile(db_file):
            ...
            if self.set == 'train' and 'mpii3d' != self.dataset_name and '3dpw' not in self.dataset_name:
                db_occ = joblib.load(db_file_occ)
                for k in db.keys():
                    db[k] = np.concatenate([db[k], db_occ[k]], axis=0)
        else:
    ...
    def get_single_item(self, index):
        ...
        if self.dataset_name == '3dpw':
            ...
            if not is_train:
                ...
                pose = self.db['pose'][start_index:end_index + 1]
                shape = self.db['shape'][start_index:end_index + 1]
                # change to torch.ones for smpl parameters supervision
                w_smpl = torch.zeros(self.seqlen).float()
                w_3d = torch.ones(self.seqlen).float()
