...
if __name__ == '__main__':
    ...
    # import poseviz
    #
    # h36m_data = np.load('/data/H36M_smpl/h36m_train_spin.npz')
    # imgname = h36m_data['imgname']
    # center = h36m_data['center']
    # scale = h36m_data['scale']
    # S = h36m_data['S']
    # pose = h36m_data['pose']
    # shape = h36m_data['shape']
    # part = h36m_data['part']
    # root = h36m_data['root']
    #
    # print(part[0, [14, 3, 4, 5, 2, 1, 0, 16, 12, 17, 18, 9, 10, 11, 8, 7, 6]])
    # for i in range(100):
    #     im = poseviz.draw_s2d(part[i, ], rule="SDK17", height=1000, width=1000)
    #     import cv2
    #     cv2.imshow('a', im)
    #     cv2.waitKey(-1)
    #
    ...
    # for i, img in enumerate(tqdm.tqdm(imgname)):
    #     img_ = img.split('/')[1].split('.')
    #     user_action = img_[0]
    #     camera, frm_id = img_[1].split('_')
    #     print(user_action, camera, frm_id)
    #     input()

    # dtype = torch.float32
    # pose = torch.tensor(pose[:128], dtype=dtype)
    # shape = torch.tensor(shape[:128], dtype=dtype)
    # smpl = SMPL(SMPL_MODEL_DIR, batch_size=128, create_transl=False)
    # output = smpl(betas=shape, body_pose=pose[:, 3:], global_orient=pose[:, :3])
    # poseviz.visual3d_seq(smpl.faces, [output.vertices])
